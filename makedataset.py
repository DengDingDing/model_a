...
def read_img(img):
    return np.array(Image.open(img).convert('RGB'))/255.0
...
def Train_data(args):
    ...
    with h5py.File(args.data_name, 'w') as h5f:
        ...
        for i in range(len(file_list)):
            print(i,'/',len(file_list),' name:',file_list[i])
            
            image = read_img(f'{args.train_path}/{args.gt_name}/{file_list[i]}')
            h,w,c = image.shape
            ...
            b,c ,h, w = img.shape
            zoom_factors = (1, 1, 256 / h, 256 / w)
            resized_img = zoom(img, zoom_factors, order=1)
            h5f.create_dataset(str(count), data=resized_img)
            count += 1

            # img = img_to_patches(img.transpose(0, 3, 1, 2), args.patch_size, args.stride, Syn=True)
            # for nx in range(img.shape[4]):
            #     data = img[:,:,:,:,nx]
            #     print(count, data.shape)
            #     # h5f.create_dataset(str(count), data=data,compression="gzip")
            #     h5f.create_dataset(str(count), data=data)
            #     count += 1
        h5f.close()

types = ["color","haze","dark","noise",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train-path", type = str, default='./data/test_set', help="Train path")
    ...
    parser.add_argument("--degradation-name", type = list, default = types, help="LQ name")
