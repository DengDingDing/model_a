...
# 	# out = net(x, query)
# 	# print(out.shape)
# 	input = torch.randn(1, 3, 111, 111).to("cuda" if torch.cuda.is_available() else "cpu")
...
if __name__ == '__main__':
    ...
    embedding = torch.randn(1, 512).to("cuda" if torch.cuda.is_available() else "cpu")
    model = OneRestore(channel=32).to("cuda" if torch.cuda.is_available() else "cpu")
    out = model(image, embedding)
    print(image.shape, embedding.shape, out.shape)
