...
class OneRestore(nn.Module):
	...
	def forward(self,x,e1,e2):
		...
		x_l, x_m, x_s, x_ss = self.encoder(x_in, e1,e2)
		x_mid = self.middle(x_ss, e1,e2)
		x_out = self.decoder(x_mid, x_ss, x_s, x_m, x_l, e1,e2)
...
class backbone(nn.Module):
	...
	def forward(self,x,e1,e2):
		...
		share1 = self.s1(x, e1,e2)
		share2 = self.s2(share1, e1,e2)
...
class decoder(nn.Module):
	...
	def forward(self, x, x_ss, x_s, x_m, x_l, e1,e2):

		dssout = self.dss(x + x_ss, e1,e2)
		x_dsin = self.conv_dsstds(self._upsample(dssout, x_s))        
		dsout = self.ds(x_dsin + x_s, e1,e2)
		x_dmin = self.conv_dstdm(self._upsample(dsout, x_m))
		dmout = self.dm(x_dmin + x_m, e1,e2)
		x_dlin = self.conv_dmtdl(self._upsample(dmout, x_l))
		dlout = self.dl(x_dlin + x_l, e1,e2)
...
class TransformerBlock(nn.Module):
	...
	def forward(self, x, e1,e2):
		x = x + self.cross_attn(self.norm1(x),e1) + self.cross_attn(self.norm1(x),e2)
...
if __name__ == '__main__':
	...
	# out = net(x, query)
	# print(out.shape)
	input = torch.randn(4, 3, 256, 256).to("cuda" if torch.cuda.is_available() else "cpu")
	query = torch.Tensor(np.random.random((4, 512))).to("cuda" if torch.cuda.is_available() else "cpu")
	query2 = torch.Tensor(np.random.random((4, 512))).to("cuda" if torch.cuda.is_available() else "cpu")

	macs, _ = profile(net, inputs=(input, query,query2))
